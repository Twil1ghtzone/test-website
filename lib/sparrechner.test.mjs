...
// Wärme: 1800 € × 11 % = 198 €
ok("Wärmeersparnis 11 % von 1800 € = 198 €", nah(r.waermeEuro, 198, 0.01), `${r.waermeEuro.toFixed(2)} €`);
// kWh dahinter: 1800 / 0,11 = 16.364 kWh, davon 11 % = 1800 kWh
ok("kWh werden aus € und Arbeitspreis abgeleitet", nah(r.waermeKwh, 1800, 1), `${e0(r.waermeKwh)} kWh`);
...
ok("Netto rund 267 €", nah(r.netto, 266.6, 1), `${r.netto.toFixed(2)} €`);
...
ok("Ersparnis = 11 % der Gesamtkosten", nah(hybrid.waermeEuro, 198, 0.01), `${hybrid.waermeEuro.toFixed(2)} €`);
...
ok("Drei Quellen funktionieren genauso", nah(drei.waermeKosten, 1800) && nah(drei.waermeEuro, 198, 0.01));
