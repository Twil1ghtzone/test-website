...
section("9) Wiederholung nur, wenn sie zeitlich ueberhaupt passt");
// Der Mock verhaelt sich wie ein langsames Reasoning-Modell: Die Dauer waechst
// mit dem Token-Budget, geliefert werden ausschliesslich Denkschritte.
const langsamerDenker = (body) => ({
  ...denkAntwort("denk denk denk"),
  delayMs: (body.max_tokens / 500) * 4000,
});

// Genug Zeit -> der zweite Versuch mit vervierfachtem Budget MUSS laufen.
m = mock(langsamerDenker);
await m.listen(++port);
r = await callAI(ai({ maxTokens: 500 }), frage, 60000);
ok("Bei viel Zeit laeuft der zweite Versuch", m.anfragen.length === 2,
   `Budgets: ${m.anfragen.map((a) => a.max_tokens).join(", ")}`);
ok("Zweites Budget ist vervierfacht", m.anfragen[1]?.max_tokens === 2000);
await m.close();

// Knappe Zeit -> der zweite Versuch wuerde am Zeitlimit scheitern und wird
// uebersprungen. Vorher pruefte der Code nur "bleiben noch 15 s?" und startete
// ihn trotzdem: Der Nutzer sass die volle Wartezeit ab und bekam nichts.
m = mock(langsamerDenker);
await m.listen(++port);
r = await callAI(ai({ maxTokens: 500 }), frage, 18000);
ok("Bei knapper Zeit nur EIN Versuch", m.anfragen.length === 1, `${m.anfragen.length} Anfrage(n)`);
ok("Antwort kommt frueh statt erst am Zeitlimit", r.ms < 17000, `${r.ms} ms von 18000 ms`);
ok("Meldung nennt die geschaetzte Dauer des zweiten Versuchs",
   r.ok === false && /gesch\u00e4tzt/.test(r.detail), r.ok === false ? r.detail.slice(0, 120) : "");
ok("Meldung nennt das Zeitlimit", r.ok === false && /Zeitlimit/.test(r.detail));
ok("Als reasoningOnly gekennzeichnet", r.ok === false && r.reasoningOnly === true);
await m.close();
